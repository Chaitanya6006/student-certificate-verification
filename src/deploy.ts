/**
 * Deploy the certificate contract to a Midnight network.
 *
 * Non-interactive: `npm run deploy -- --network preview` runs straight
 * through. Prints the wallet address + faucet hint and WAITS (up to 10 min)
 * for funding when the wallet has no tNIGHT, then deploys and records the
...
// certificate contract has no `witness` declarations — every private input is
// a circuit parameter — so its private state is empty ({}).
...
async function main() {
  console.log('\n╔══════════════════════════════════════════════════════════════╗');
  console.log(`║  Deploy Certificate Registry to ${network}`);
  console.log('╚══════════════════════════════════════════════════════════════╝\n');
  ...
  console.log(`  adminHash: 0x${Buffer.from(admin.adminHash).toString('hex')}`);
  console.log('  (the admin secret itself is only on this machine — never on-chain)\n');
